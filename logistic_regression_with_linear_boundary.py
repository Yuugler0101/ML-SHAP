...
precision = np.sum(y_train_prections == y_train)/y_train.shape[0] * 100  #准确率（百分比）=预测对的值/总的值
print ('precision:',precision)

#测试结果
x_min = np.min(x_train[:,0])  #x_train的第一个轴（x_axis）的最小值
x_max = np.max(x_train[:,0])
y_min = np.min(x_train[:,1])
y_max = np.max(x_train[:,1])
samples= 150 
X = np.linspace(x_min,x_max,samples)
Y = np.linspace(y_min,y_max,samples)   #在指定的起始点和结束点之间生成等间隔的数值序列

Z_SETOSA = np.zeros((samples,samples))
Z_VERSICOLOR = np.zeros((samples,samples))
Z_VIRGINICA = np.zeros((samples,samples))   #初始化150*150的矩阵
